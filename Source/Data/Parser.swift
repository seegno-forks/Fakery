...
open class Parser {
  ...
  func parse(_ template: String, forSubject subject: String) -> String {
    ...
    do {
      ...
      let matches = regex.matches(in: templateString as String,
      ...
      if matches.count > 0 {
        for match in matches {
          ...
          if prefixRange.length > 0 {
            text += templateString.substring(with: prefixRange)
          }

            var subjectWithDot: String = subject + "."
          if subjectRange.length > 0 {
            subjectWithDot = templateString.substring(with: subjectRange)
          }

          if methodRange.length > 0 {
            let key = subjectWithDot.lowercased() + templateString.substring(with: methodRange)
            text += fetch(key)
          }

          if otherRange.length > 0 {
            text += templateString.substring(with: otherRange)
          }
        }
      } else {
        text = template
